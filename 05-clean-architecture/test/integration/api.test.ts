...
test("Deve executar uma orderm de compra com uma ordem de venda", async () => {
  ...
  const responsePlaceOrderBuy = await fetch(
  ...
  const responsePlaceOrderSell = await fetch(
    "http://localhost:3000/place-order",
    {
      method: "POST",
      headers: {
  ...
  const responseGetOrderBuy = await fetch(
    `http://localhost:3000/orders/${outputPlaceOrderBuy.orderId}`,
  );
  const outputGetOrderBuy = await responseGetOrderBuy.json();

  const responseGetOrderSell = await fetch(
    `http://localhost:3000/orders/${outputPlaceOrderSell.orderId}`,
  );
