...
test("Deve executar uma orderm de compra com uma ordem de venda", async () => {
  ...
  const outputGetOrderSell = await getOrder.execute(
    outputPlaceOrderSell.orderId,
  );
...
test("Deve executar uma orderm de compra com duas ordens de venda", async () => {
  ...
  const outputGetOrderBuy = await getOrder.execute(outputPlaceOrderBuy.orderId);
  const outputGetOrderSell1 = await getOrder.execute(
    outputPlaceOrderSell1.orderId,
  );
  const outputGetOrderSell2 = await getOrder.execute(
    outputPlaceOrderSell2.orderId,
  );
