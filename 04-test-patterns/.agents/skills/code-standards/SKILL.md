...
6. **Avoid unnecessary comments.** A comment that just restates the code is noise that
   ...
   comments that explain _why_ something non-obvious is done, not _what_ the line does.
...
- **Don't over-correct.** These are heuristics for readability, not laws to enforce
  mechanically. If applying a rule literally would make the code _less_ clear (e.g.
